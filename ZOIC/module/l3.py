            ...
            def send_packets(target_ip, byte_size, count):
                ...
                with socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP) as sock:
                    for i in range(count):
                        ...
                        print(TextColors.GREEN + f'[+] IP Address : {target_ip} [+] ICMP Packet : {i + 1}'+ TextColors.RESET)
                        time.sleep(0.01)

            target = input(TextColors.LIGHT_RED + "IP > "+ TextColors.RESET)
            byte_size = int(input(TextColors.LIGHT_RED + "Bytes > "+ TextColors.RESET))
            count = int(input(TextColors.LIGHT_RED + "Count > "+ TextColors.RESET))
            send_packets(target, byte_size, count)

        elif select == "2" or select.lower() == "2":

            ip = input(TextColors.LIGHT_RED +"IP > "+ TextColors.RESET)
            Bytes = input(TextColors.LIGHT_RED +"Bytes > "+ TextColors.RESET)
